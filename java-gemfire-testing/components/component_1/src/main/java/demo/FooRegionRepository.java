...
public class FooRegionRepository {
    private Region<String, User> fooRegion;
    ...
    @Autowired
    public FooRegionRepository(@Value("#{FooRegion}") Region<String, User> fooRegion) {
        this.fooRegion = fooRegion;
    }

    public List<User> getAllValues() {
        try {
            SelectResults<User> valuesQuery = fooRegion.query("select * from /FooRegion");
